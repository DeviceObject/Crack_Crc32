...
int main(int argc, const char * argv[])
{
    ...
    if (argc == 2) {
        if (0 != argv[1]) {
            ...
            size = (int)ftell(fpHdr);
            if (0 == size) {
                printf("main(): get file size error.\r\n");
                fclose(fpHdr);
                return -1;
            }
            fseek(fpHdr, 0, SEEK_SET);
            tmp_size = size;
            ...
            if (0 == fread(crack_thread_info.szDat, size, 1, fpHdr)) {
            ...
            tmp_size -= 0x08;
            crack_thread_info.check_crc32 = (int)*(long *)((long)crack_thread_info.szDat + tmp_size);
            if (crack_thread_info.check_crc32 > 0) {
                do {
                    crack_thread_info.ThreadParamters[crack_thread_info.thread_count].calc_length = tmp_size;
